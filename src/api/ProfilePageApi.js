...
const staggeredBaseQuery = async (args, api, extraOptions) => {
    const result = await fetchBaseQuery({
        baseUrl: "https://api2.daybestgroup.com",
        // prepareHeaders: () => {
...
export const ProfilePageApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getAttendanceList: builder.query({
            query: ({ date, page, size }) =>
                `/admin/attendence-list/?date=${date}&page=${page}&size=${size}`,
        }),
        ...
        getEmployeeSearch: builder.query({
            query: ({ page, size, key }) =>
                `/admin/employees-search/?page=${page}&size=${size}&key=${key}`,
            providesTags: ["employee-list"],
        }),
        getInventoryList: builder.query({
            query: ({ page, size }) =>
                `/admin/items-list?page=${page}&size=${size}`,
